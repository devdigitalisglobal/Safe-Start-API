...
import {
  latestCompletedAttemptsByUser,
  scoresByKnowledgeAreaForAttempts,
  pctFromAreaScores,
} from './helpers.js';
...
export async function computeLearning(
  ...
  let userIds: string[];
  let sgAssessment: { id: string } | null;
  let flAssessment: { id: string } | null;

  if (shared) {
    userIds = shared.userIds;
    sgAssessment = shared.sgAssessment;
    flAssessment = shared.flAssessment;
  } else {
    const studentScope = studentWhere(scope);
    const [students, sg, fl] = await Promise.all([
...
export async function computeImprovement(
  ...
  let areas: { id: string; key: string; name: string }[];
  let sgAssessment: { id: string } | null;
  let flAssessment: { id: string } | null;

  if (shared) {
    areas = shared.knowledgeAreas;
    sgAssessment = shared.sgAssessment;
    flAssessment = shared.flAssessment;
  } else {
    const [loadedAreas, sg, fl] = await Promise.all([
